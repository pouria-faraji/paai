...
class QueryDataBaseService(object):
    ...
    @staticmethod
    async def get_aggregation_by_device_id(db: AsyncIOMotorDatabase,
                                       device_id: str,
                                       since:datetime = None,
                                       to:datetime = None, 
                                       operation = 'avg'):
        """Creating a pipeline to calculate average/maximum/minimum, in a time frame
        """
        # Defining default aggregation in case operation parameter is None or irrelevant
        fieldName = 'totalAverage'
        groupVariable = '$avg'

        if operation == 'max':
            fieldName = 'totalMaximum'
            groupVariable = '$max'
        elif operation == 'min':
            fieldName = 'totalMinimum'
            groupVariable = '$min'
        ...
        pipeline = [
            ...
            {
                '$group': {'_id': None, fieldName: {groupVariable: groupVariable}}
            }
        ]
        cursor = aggregate(db, COLLECTION_NAME, pipeline)
        results = []
